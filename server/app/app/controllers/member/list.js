const { dtoIn } = require("../../dto/member/list-dto");
const Errors = require("../../errors");
const BaseError = require("../../errors/BaseError");
const { checkAuthorization, isListOwner, isListMember } = require("../../utils/authorization");
const listHandler = require("../../commandHandlers/member/list");
...
async function handler(ucEnv, dtoInData) {
  ...
  try {
    ...
    throw new Errors.Member.List.InvalidDtoIn(dtoInData, error);
  }

  // Get shopping list
  const shoppingList = await ShoppingList.get(ucEnv, validatedDtoIn.listId);
  if (!shoppingList) {
    throw new Errors.Member.List.ShoppingListNotFound(validatedDtoIn);
  }

  // Check authorization - Authorities, ListOwner, or ListMember can list members
  const isAuthorized = checkAuthorization(ucEnv, ["Authorities"]) ||
    isListOwner(ucEnv, shoppingList) ||
    isListMember(ucEnv, shoppingList);

  if (!isAuthorized) {
    throw new Errors.Member.List.Unauthorized(validatedDtoIn);
  }
