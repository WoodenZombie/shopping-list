const { dtoIn } = require("../../dto/item/remove-dto");
const Errors = require("../../errors");
const BaseError = require("../../errors/BaseError");
const { checkAuthorization, isListOwner, isListMember } = require("../../utils/authorization");
const removeHandler = require("../../commandHandlers/item/remove");
...
async function handler(ucEnv, dtoInData) {
  ...
  try {
    ...
    throw new Errors.Item.Remove.InvalidDtoIn(dtoInData, error);
  }

  // Get shopping list
  const shoppingList = await ShoppingList.get(ucEnv, validatedDtoIn.listId);
  if (!shoppingList) {
    throw new Errors.Item.Remove.ShoppingListNotFound(validatedDtoIn);
  }

  // Get item
  const item = await Item.get(ucEnv, validatedDtoIn.listId, validatedDtoIn.id);
  if (!item) {
    throw new Errors.Item.Remove.ItemNotFound(validatedDtoIn);
  }

  // Check authorization - Authorities, ListOwner, or ListMember can remove items
  const isAuthorized = checkAuthorization(ucEnv, ["Authorities"]) ||
    isListOwner(ucEnv, shoppingList) ||
    isListMember(ucEnv, shoppingList);

  if (!isAuthorized) {
    throw new Errors.Item.Remove.Unauthorized(validatedDtoIn);
  }
