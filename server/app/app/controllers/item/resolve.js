const { dtoIn } = require("../../dto/item/resolve-dto");
const Errors = require("../../errors");
const BaseError = require("../../errors/BaseError");
const { checkAuthorization, isListOwner, isListMember } = require("../../utils/authorization");
const resolveHandler = require("../../commandHandlers/item/resolve");
...
async function handler(ucEnv, dtoInData) {
  ...
  try {
    ...
    throw new Errors.Item.Resolve.InvalidDtoIn(dtoInData, error);
  }

  // Get shopping list
  const shoppingList = await ShoppingList.get(ucEnv, validatedDtoIn.listId);
  if (!shoppingList) {
    throw new Errors.Item.Resolve.ShoppingListNotFound(validatedDtoIn);
  }
  ...
  if (!item) {
    throw new Errors.Item.Resolve.ItemNotFound(validatedDtoIn);
  }

  // Check authorization - Authorities, ListOwner, or ListMember can resolve items
  const isAuthorized = checkAuthorization(ucEnv, ["Authorities"]) ||
    isListOwner(ucEnv, shoppingList) ||
    isListMember(ucEnv, shoppingList);

  if (!isAuthorized) {
    throw new Errors.Item.Resolve.Unauthorized(validatedDtoIn);
  }

  // Call command handler
  let dtoOut;
  try {
    dtoOut = await resolveHandler(ucEnv, validatedDtoIn);
  } catch (error) {
